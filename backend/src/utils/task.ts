...
export const getCarIdByAccountId = async (
  accountId: string
): Promise<string> => {
  try {
    if (!accountId) {
...
export const getProductsByBinId = async (
  binID: string
): Promise<{ productID: string; quantity: number; inventoryID: string }[]> => {
  try {
    if (!binID) return []
...
export const checkExistingInventory = async (
  binID: string,
  productID: string
): Promise<boolean> => {
  try {
    ...
    console.error(
      `❌ Error checking inventory for bin ${binID} and product ${productID}:`,
      error
    )
    return false
  }
