...
import User from '../models/account' // ✅ 新增 User Model 引用
...
export const loadCargo = async (
  ...
  try {
    ...
    if (!user || !user.CarID) {
      res
        .status(400)
        .json({ message: '❌ User does not have a CarID assigned' })
      return
    }
...
export const unloadCargo = async (
  ...
  try {
    ...
    if (!Array.isArray(productList) || productList.length === 0) {
      res
        .status(400)
        .json({ message: '❌ Product list is required for unloading' })
      return
    }
    ...
    if (updatedCount === 0) {
      res
        .status(404)
        .json({ message: '❌ No matching products found to update' })
      return
    }
...
export const scanPickerareaBin = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    const pickerBin = await Bin.findOne({
      where: { binID: pickerBinID },
    ...
    if (!pickerBin || !pickerBin.productID) {
      res
        .status(400)
        .json({ message: '❌ pickerBinID must have a predefined productID.' })
      return
    }
    ...
    if (existingPickerBin) {
      ...
      await Inventory.destroy({
        where: { inventoryID: productInCar.inventoryID }
      })
...
export const getUserTaskStatus = async (
  ...
  try {
    ...
    if (!task) {
      res
        .status(200)
        .json({ status: 'completed', currentBinID: null, productList: [] }) // 默认状态为 completed
      return
    }
